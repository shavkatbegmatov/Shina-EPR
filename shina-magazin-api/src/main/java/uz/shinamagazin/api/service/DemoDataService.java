            ...
            SELECT
              (SELECT COUNT(DISTINCT sale.id)
                ...
                WHERE sale.invoice_number NOT LIKE 'DEMO-S-%'
                  AND (customer.notes = '[PROTEKTOR_DEMO]' OR product.sku LIKE 'DEMO-%'))
              +
              (SELECT COUNT(DISTINCT purchase.id)
                ...
                WHERE purchase.order_number NOT LIKE 'DEMO-P-%'
                  AND (supplier.notes = '[PROTEKTOR_DEMO]' OR product.sku LIKE 'DEMO-%'))
              +
              (SELECT COUNT(DISTINCT shop.id)
                 FROM shop_orders shop
                 LEFT JOIN customers customer ON customer.id = shop.customer_id
              ...
              (SELECT COUNT(*)
                 FROM debts debt
                 JOIN customers customer ON customer.id = debt.customer_id
                WHERE customer.notes = '[PROTEKTOR_DEMO]'
                  AND debt.notes IS DISTINCT FROM '[PROTEKTOR_DEMO]')
              +
              (SELECT COUNT(*)
                 FROM stock_movements movement
                 JOIN suppliers supplier ON supplier.id = movement.supplier_id
    ...
    private void ensureNoExternalReferences() {
        ...
        if (referenceCount != null && referenceCount > 0) {
            throw new ResponseStatusException(
                    HttpStatus.CONFLICT,
                    "Demo obyektlariga bog'langan oddiy hujjatlar mavjud. "
